...
def compute_leo_metrics(folder_path: str, source_path: str) -> dict:
    base_name = os.path.basename(folder_path.rstrip('/\\'))
    results_dir = os.path.join(source_path, f"{base_name}_results_dd")
...
def compute_neosca_sca_metrics(text_content: str):
...
ALL_OUTPUT_FIELDS = [
    ...
    "MHD", "TDL", "SL", "MV", "VK", "MTW", "MTH",
    "HI", "HF", "MTDL", "MSL",
    "MLS", "MLT", "MLC", "C/S", "VP/T", "C/T",
    "DC/C", "DC/T", "T/S", "CT/T", "CP/T",
    "CP/C", "CN/T", "CN/C",
...
def process_category_folder(category_path: str, output_dir: str, nlp, source_dir):
    ...
    if not os.path.exists(category_path):
        print(f"Folder path {category_path} does not exist, processing next folder.")
        return
    ...
    base_name = os.path.basename(category_path)
    leo_results_dir = os.path.join(source_dir, f"{base_name}_results_dd")
    ...
    if os.path.exists(leo_csv_path):
        try:
            ...
            leo_metrics, leo_results_dir = compute_leo_metrics(category_path, source_dir)
            leo_csv_path = os.path.join(leo_results_dir, "0mdd_ndd_results.csv")
    else:
    ...
    with open(output_csv, file_mode, newline="", encoding="utf-8") as csvfile:
        ...
        for txt_file in txt_files:
            ...
            ns_metrics = compute_neosca_sca_metrics(text_content)
